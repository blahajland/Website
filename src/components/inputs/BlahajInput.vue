...
function validateString(event) {
  ...
  if (value === '') {
    val = props.isMandatory ? VALIDATOR_STATE.INVALID : VALIDATOR_STATE.NONE
...
<template>
  <div class="BlahajInput" :class="{valid : validator === 1, invalid : validator === 2}">
    <div>
      <p>{{ placeholder }} {{ isMandatory ? '&bull; mandatory' : '' }}</p>
